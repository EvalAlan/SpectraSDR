...
const { migrateLegacyDataRoot, legacyDataRootFor } = require('./migrate-data');

const SERVER_PORT = 5555;
const SERVER_HOST = '127.0.0.1';
const BACKEND_DIR = app.isPackaged
  ? path.join(process.resourcesPath, 'SpectraSDR')
  : path.join(__dirname, '..', '..');
...
const DATA_ROOT = path.join(app.getPath('userData'), 'SpectraSDR');
const LEGACY_DATA_ROOT = legacyDataRootFor(app.getPath('userData'));

let mainWindow = null;
let backendProcess = null;

function ensureDataFiles() {
  migrateLegacyDataRoot(DATA_ROOT, LEGACY_DATA_ROOT);
...
function startBackend() {
  ...
  backendProcess = spawn(python, [BACKEND_ENTRY], {
    env: {
      ...process.env,
      PYTHONUNBUFFERED: '1',
      SPECTRASDR_DATA_ROOT: DATA_ROOT,
      SPECTRASDR_CONFIG_FILE: path.join(DATA_ROOT, 'config.json'),
      SPECTRASDR_BOOKMARKS_FILE: path.join(DATA_ROOT, 'bookmarks.json'),
      SPECTRASDR_CONNECTIONS_FILE: path.join(DATA_ROOT, 'connections.json'),
      SPECTRASDR_METADATA_PREFS_FILE: path.join(DATA_ROOT, 'metadata_prefs.json'),
      SPECTRASDR_RECORDINGS_DIR: path.join(DATA_ROOT, 'recordings')
    },
    cwd: BACKEND_DIR,
  ...
  backendProcess.on('exit', code => {
    ...
    if (!app.isQuitting && code !== 0) {
      dialog.showErrorBox('SpectraSDR', `Backend terminated unexpectedly (code ${code}).`);
      app.quit();
    }
  });
...
app.whenReady().then(() => {
  startBackend()
    .then(() => createWindow())
    .catch(err => {
      dialog.showErrorBox('SpectraSDR', `Failed to start backend: ${err.message}`);
      app.quit();
    });
});

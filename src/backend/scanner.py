...
class Scanner:
    ...
    def __init__(self, rtl_client, dsp, bookmarks_file=None):
        ...
        self._on_hit: typing.Optional[typing.Callable[[dict], typing.Any]] = None
    ...
    def load_bookmark_freqs(self, category_name=None):
        """Load bookmark frequencies from file, optionally filtered by tag/category name."""
        self.bookmark_freqs = []
        wanted = (category_name or "").strip().lower()
        try:
            if self.bookmarks_file.exists():
                data = json.loads(self.bookmarks_file.read_text())

                if isinstance(data, dict) and isinstance(data.get("bookmarks"), list):
                    for station in data.get("bookmarks", []):
                        tags = [str(t).strip() for t in (station.get("tags") or []) if str(t).strip()]
                        if wanted and wanted not in {t.lower() for t in tags}:
                            continue
                    ...
                    for tag in data.get("tags", []):
                        ...
                        if wanted and tag_name.lower() != wanted:
                            continue
                        for station in tag.get("stations", []):
                            station_tags = [str(t).strip() for t in (station.get("tags") or []) if str(t).strip()]
                            if tag_name and tag_name not in station_tags:
                                station_tags.append(tag_name)
                            self.bookmark_freqs.append({
                                "freq": station.get("frequency", 0),
                                "mode": station.get("mode", "FM"),
                                "label": station.get("label", ""),
                                "category": ", ".join(station_tags),
                            })

                else:
                    # Legacy category format
                    for cat in data.get("categories", []) if isinstance(data, dict) else []:
                        cat_name = str(cat.get("name", "")).strip()
                        if wanted and cat_name.lower() != wanted:
                            continue
            ...
            self.bookmark_freqs = [b for b in self.bookmark_freqs if int(b.get("freq", 0)) > 0]
            cat_msg = f" (tag: {category_name})" if category_name else " (all tags)"
    ...
    def get_categories(self):
        """Return tag/category names from bookmarks file (legacy method name kept for API compatibility)."""
        try:
            if self.bookmarks_file.exists():
                data = json.loads(self.bookmarks_file.read_text())
                if isinstance(data, dict) and isinstance(data.get("bookmarks"), list):
                    tags = set()
                    for station in data.get("bookmarks", []):
                        for tag in station.get("tags", []) or []:
                            name = str(tag).strip()
                            if name:
                                tags.add(name)
                    return sorted(tags)
                    ...
                    if signal_db > self.dsp.squelch_threshold:
                        ...
                        if self._on_hit:
                            try:
                                ...
                                maybe_coro = self._on_hit(hit)
                                if asyncio.iscoroutine(maybe_coro):
                                    await maybe_coro
                            except Exception:
                                logger.exception("Scanner hit callback failed")

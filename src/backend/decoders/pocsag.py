#!/usr/bin/env python3
"""POCSAG (Pager) decoder for SpectraSDR.
...
class POCSAGDecoder(BaseDecoder):
    ...
    def __init__(self, sample_rate=48000):
        ...
        self._recent_signatures = deque(maxlen=128)
    ...
    def _try_decode_baud(self, baud_rate):
        ...
        # Bit slicing using per-bit mean to reduce phase jitter sensitivity
        bit_values = []
        for i in range(n_bits):
            ...
            if end <= start:
                continue
            bit_values.append(1 if np.mean(self._sample_buffer[start:end]) > 0 else 0)
        bits = np.array(bit_values, dtype=int)
    ...
    def _decode_batch(self, bits, baud_rate):
        ...
        for frame_idx in range(8):
            for cw_idx in range(2):
                ...
                # Repair before classifying: a single bit error in an IDLE
                # codeword is trivially correctable, but left uncorrected it
                # reads as a valid address codeword and injects a phantom
                # address into the stream.
                ...
                if is_message == 0: # Address codeword
                    ...
                    addr_bits = (codeword >> 13) & 0x3FFFF # 18 bits
    ...
    def _emit_message(self, address, function_bits, data_bits, baud_rate):
        ...
        if is_alpha:
            content = alpha_text.strip()
            msg_type = "alpha"
        else:
            content = numeric_text.strip()
            msg_type = "numeric"

        if len(content) < 2:
            return

        msg = POCSAGMessage(address, function_bits, content, msg_type, baud_rate)

        signature = (msg.address, msg.function_bits, msg.content, msg.msg_type)
        ...
        if not is_duplicate and msg.content:
            ...
            self.emit(DecoderResult(
                decoder=self.name,
                type="pager",
                summary=f"[{msg.msg_type}] {msg.address}: {msg.content[:40]}",
                data=msg.to_dict(),
            ))
...
def _bch_syndrome(data31):
    """Remainder of the 31-bit BCH word divided by the generator polynomial."""
    remainder = data31
...
def _build_syndrome_table():
    """Map every correctable syndrome to the error pattern that produced it.

    The syndrome is linear -- syndrome(word ^ err) == syndrome(err) for any valid
    word -- so enumerating all weight-1 and weight-2 patterns up front turns
    correction into a dict lookup. BCH(31,21) has minimum distance 5, so those
    496 patterns map to 496 distinct syndromes (asserted in the tests) and the
    ...
    for a in range(31):
        for b in range(a, 31):
            err = (1 << a) | (1 << b)
            table.setdefault(_bch_syndrome(err), err)
    return table

_BCH_SYNDROME_TABLE = _build_syndrome_table()
...
def _bch_correct(codeword):
    """Correct up to two bit errors in a 32-bit codeword, or None if beyond reach."""
    data = codeword >> 1
    observed_parity = codeword & 1

    syndrome = _bch_syndrome(data)
    if syndrome == 0:
        # BCH section is intact, so the parity bit itself must be the error.
        fixed, weight = data, 0
    else:
        err = _BCH_SYNDROME_TABLE.get(syndrome)
        if err is None:
            return None
        fixed = data ^ err
        weight = bin(err).count('1')

    # Rebuild the parity bit, counting it as a third error if it disagrees with
    # what we received. Past two errors this code can only guess, so reject
    # rather than hand back a confident-looking miscorrection.
    parity = bin(fixed).count('1') % 2
    if weight + (parity != observed_parity) > 2:
        return None
    return (fixed << 1) | parity
